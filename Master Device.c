...
#define slave_rdy_AD ANSELDbits.ANSD2
#define slave_rdy PORTDbits.RD2
#define slave_rdy_dir TRISDbits.TRISD2
...
unsigned int map(double x, double in_min, double in_max, double out_min, double out_max) {
...
PWM2_Duty(unsigned int horizontal,unsigned int vertical)// This is for Brushless Motors
{     //red
    M1HOR=0;
    M2HOR=0;
    M1VERT=0;
  ...
  {
        //duty=duty-510;
        horiz = (((float)horizontal-500)/520)*(_XTAL_FREQ/(PWM_freq*TMR2PRESCALE)); // On reducing //duty = (((float)duty/1023)*(1/PWM_freq)) / ((1/_XTAL_FREQ) * TMR2PRESCALE);
        //The reason we divide by 1666 is to reduce the maximum power of the motors for only left or right turn with no throttle 30%. Plus this 30% and 70% throttle make 100% 
        M1HOR=M1HOR-horiz;
        M2HOR=M2HOR+horiz;
        
        if (M1HOR<0){ // In the cases where we are doing a left turn, M1 should be 1.5ms and M2> 1.5ms 
            M1HOR=0;
        }
        if (M2HOR<0){
            M2HOR=0;
        }
        M1HOR=map(M1HOR, 0, 625, 46.88, 62.5);//This should map the value of M1 to create a pulse width between 1.5ms to 2ms 
        M2HOR=map(M2HOR, 0, 625, 46.88, 62.5);//This should map the value of M2 to create a pulse width between 1.5ms to 2ms 

        
  }
      else if(horizontal<500 && horizontal>=0){
        horiz = (((((float)horizontal)-500)*-1)/520)*(_XTAL_FREQ/(PWM_freq*TMR2PRESCALE)); // On reducing //duty = (((float)duty/1023)*(1/PWM_freq)) / ((1/_XTAL_FREQ) * TMR2PRESCALE);
        ...
        /*********************************************Vertical Control*********************************************************************************/
      if(vertical<1025&&vertical>0)
  {
        //duty=duty-510;
        vert = (((float)vertical)/1025)*(_XTAL_FREQ/(PWM_freq*TMR2PRESCALE)); // On reducing //duty = (((float)duty/1023)*(1/PWM_freq)) / ((1/_XTAL_FREQ) * TMR2PRESCALE);
        
        M1VERT=M1VERT+vert;
        M2VERT=M2VERT+vert;
        
        M1VERT=map(M1VERT, 0, 625, 31.25, 62.6);//This should map the value of M1 to create a pulse width between 1ms to 2ms 
        M2VERT=map(M2VERT, 0, 625, 31.25, 62.6);//This should map the value of M2 to create a pulse width between 1ms to 2ms 

        if (M1HOR<47.5){
            M1T=(M1HOR*0.4)+(M1VERT*0.6);
            M2T=(M2HOR*0.4) +(M2VERT*0.6);
...
AD_Initialize(){
    ...
    ADCLK=0b00011111;//FOSC=4MHz, ADC clock source= FOSC/4= 1us
...
void Serial_comm_init(void){
    ...
    ANSC3=0;
    TRISC3=0;//SET RC3 AS INPUT check
...
int main(int argc, char** argv) {
    ...
    OSCFRQbits.HFFRQ=0b010; // This will set the oscillator to 4MHZ if RSTOSC = HFINT32
    ...
    /*Set up RA5 as external interrupt button (active high)*/
    INTPPS=0x20;
    ...
    ANSD2=0;
    TRISD2=0;
    /*This is to set up the PWM and timer 4*/
    /*
    T4CLKCONbits.T4CS=0b00000001;// Input clock to timer 4 is FOSC/4
    T4CONbits.T4ON=1;
    T4CONbits.T4CKPS=0b111;//prescaler =128
    T4CONbits.T4OUTPS=0b0000;
    T4HLTbits.T4PSYNC=0;
    
    
    PR4=0xFF;
    PIE4bits.TMR4IE=1;//TMR4 to PR4 Match Interrupt Enable bit
    PIR4bits.TMR4IF=0;//Timer4 Interrupt Flag bit
    */
    ...
    while(1){
        //PWM2_Duty(channel1,channel2);
        sendSPI(254);
        
        delayby_s;
        sendSPI(254);
        //TMR4IF=1;
        //PWM1_Duty(channel1,channel2);
...
void __interrupt(high_priority) tcInt(void){
    ...
    else if (TMR4IE==1 && TMR4IF==1){
        PIR4bits.TMR4IF=0;
        if (ct2==30){//if FOSC=32MHZ then change to ct2=245
            ct2=0;
            if ( PORTAbits.RA1 ==1){
            PORTAbits.RA1=0 ;
            }
